...
const KnowledgeTree = () => {
    const handleToggleNode = (e) => {
        e.currentTarget.closest('.mnode').classList.toggle('open');
    };
    ...
    return (
        <div className="guide-container pt-20 pb-16 min-h-screen">
            <style dangerouslySetInnerHTML={{__html: `
.guide-container{
  --disp:'League Spartan',ui-sans-serif,sans-serif;
  --sans:'Josefin Sans',ui-sans-serif,sans-serif;
}
/* LIGHT (default) — warm ivory/sand */
.guide-container{
  --bg:linear-gradient(180deg,#F3EADB 0%,#EFE6D6 100%);
  --panel:#FBF4E9;
  --panel2:#F5ECDD;
  --panel3:#EFE4D2;
  --ink:#2A2318;
  ...
  --gold-bg:rgba(169,121,30,.09);
  --sindoor:#C24A26;
  --sindoor-bg:rgba(194,74,38,.08);
  --green:#2E7D53;
  --line:#E2D6C2;
  --line-2:#D8C9B0;
  --shadow:0 2px 6px rgba(120,86,40,.06),0 6px 18px rgba(120,86,40,.08);
  --node:#FBF4E9;
}
/* DARK */
[data-theme="dark"] .guide-container{
  --bg:radial-gradient(1000px 600px at 20% -10%,#1a2033 0%,transparent 55%),#0B0E1A;
  --panel:#12172A;
  --panel2:#171D33;
  --panel3:#1E2740;
  --ink:#F2EBDB;
  ...
  --faint:#7A8290;
  --gold:#E8B44A;
  --gold-dim:rgba(232,180,74,.35);
  --gold-bg:rgba(232,180,74,.10);
  --sindoor:#E4572E;
  --sindoor-bg:rgba(228,87,46,.12);
  --green:#7FC98F;
  --line:rgba(232,180,74,.18);
  --line-2:rgba(232,180,74,.28);
  --shadow:0 4px 14px rgba(0,0,0,.4);
  --node:#171D33;
}
.guide-container {background:transparent;color:var(--body);font-family:var(--sans);font-weight:400;line-height:1.65;transition:background .4s,color .4s;min-height:100vh}
.about-container .guide-container ::selection{background:var(--sindoor);color:#fff}
...
/* master flowchart */
.mapblock{margin:30px 0 16px;background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:26px 16px 14px;box-shadow:var(--shadow)}
...
.mapblock svg text{font-family:'Josefin Sans',sans-serif}
...
..guide-container {padding:14px 8px 4px 18px;border-left:2px dashed var(--line);margin:10px 0 0 10px}
...
/* ============ INTERACTIVE NODE-MAP (replaces static SVG) ============ */
.imap-hint{text-align:center;font-family:var(--sans);font-size:.8rem;color:var(--faint);font-style:italic;margin-bottom:16px;display:flex;align-items:center;justify-content:center;gap:7px}
.imap-hint svg{width:15px;height:15px}

.imap{display:flex;flex-direction:column;align-items:center;gap:0;position:relative}

/* a node in the map */
.mnode{position:relative;display:flex;flex-direction:column;align-items:center;width:100%}
.mbtn{position:relative;z-index:2;display:inline-flex;flex-direction:column;align-items:center;gap:3px;
  background:var(--node);border:1.5px solid var(--line-2);border-radius:14px;padding:12px 20px;cursor:pointer;
  box-shadow:var(--shadow);transition:transform .2s,border-color .2s,background .2s;text-align:center;max-width:320px}
.mbtn:hover{transform:translateY(-2px);border-color:var(--gold-dim)}
.mbtn:active{transform:scale(.97)}
.mbtn .mt{font-family:var(--disp);font-weight:700;font-size:1rem;color:var(--ink);letter-spacing:.06em}
.mbtn .ms{font-size:.62rem;color:var(--faint);letter-spacing:.05em;text-transform:uppercase;font-weight:600}
/* root & trunk variants */
.mbtn.root{background:var(--gold-bg);border-color:var(--gold);border-width:2px}
.mbtn.root .mt{color:var(--gold);font-size:1.15rem;letter-spacing:.12em}
.mbtn.trunk{background:var(--sindoor-bg);border-color:var(--sindoor)}
.mbtn.trunk .mt{color:var(--sindoor)}
/* leaf level */
.mbtn.leaf{padding:9px 15px}
.mbtn.leaf .mt{font-size:.82rem;font-weight:600}
/* expand chevron */
.mbtn .mchev{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:16px;height:16px;color:var(--gold);opacity:.6;transition:transform .3s}
.mbtn.has-children{padding-right:30px}
.mnode.open>.mbtn .mchev{transform:translateY(-50%) rotate(180deg)}
/* jump icon on terminal nodes */
.mbtn .mjump{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:14px;height:14px;color:var(--green);opacity:.7}
.mbtn.jumpable{padding-right:28px}

/* connector line down from a node to its children */
.mconn{width:2px;background:var(--line-2);height:0;transition:height .35s ease}
.mnode.open>.mconn{height:22px}

/* children container */
.mchildren{display:flex;gap:14px;flex-wrap:wrap;justify-content:center;align-items:flex-start;
  max-height:0;overflow:hidden;opacity:0;transition:max-height .4s ease,opacity .35s ease,padding .3s;padding:0}
.mnode.open>.mchildren{max-height:4000px;opacity:1;padding:6px 4px 0}
/* horizontal connector rail above a row of children */
.mchildren.railed{position:relative;padding-top:20px}
.mchildren.railed::before{content:"";position:absolute;top:0;left:12%;right:12%;height:2px;background:var(--line-2)}

/* each child column has its own little stem */
.mnode.child{width:auto;min-width:0}
.mnode.child>.mbtn{margin-top:0}
.mnode.child::before{content:"";position:absolute;top:-20px;left:50%;width:2px;height:20px;background:var(--line-2);transform:translateX(-50%)}
.mchildren:not(.railed) .mnode.child::before{display:none}

/* highlight flash when you jump to a definition */
@keyframes flashcard{0%{background:var(--gold-bg);border-color:var(--gold)}100%{background:var(--node);border-color:var(--line)}}
.flash>summary{animation:flashcard 1.6s ease}

/* section anchor offset for sticky-free jump */
...
@media (max-width:760px){
  ...
  .mapblock{padding:18px 10px 12px;border-radius:16px}
  /* map: tighter, allow horizontal scroll inside children rows if needed */
  .mchildren{gap:10px}
  .mbtn{padding:10px 16px;max-width:76vw}
  .mbtn.has-children{padding-right:28px}
  .mbtn .mt{font-size:.92rem}
  .trunk{padding:40px 0 8px}
  .trunk h2{font-size:1.7rem}
  /* content tables scroll */
  table{display:block;overflow-x:auto;white-space:nowrap}
  .keys{grid-template-columns:1fr}
  summary{flex-wrap:wrap}
  summary .meta{margin-left:0;width:100%;margin-top:4px}
}

`}} />
            {/* Main content from guide page */}
...
<h2>Interactive Canon Map — tap to expand, tap a text to jump to its definition</h2>
<div className="imap-hint"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3 8-8"/><path d="M20 12v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h9"/></svg> Tap any node to open its branches · tap again on a text to leap to its full entry below</div>

<div className="imap" id="imap">
  {/* ROOT */}
  <div className="mnode open" data-level="root">
    <button className="mbtn root has-children" onClick={handleToggleNode}>
      <span className="mt">SANĀTANA DHARMA</span>
      <span className="ms">Vāṅmaya · the total sacred literature</span>
      <svg className="mchev" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
    </button>
    <div className="mconn"></div>
    <div className="mchildren railed">

      {/* ============ ŚRUTI TRUNK ============ */}
      <div className="mnode child" data-level="trunk">
        <button className="mbtn trunk has-children" onClick={handleToggleNode}>
          <span className="mt">ŚRUTI</span><span className="ms">Heard · revealed</span>
          <svg className="mchev" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
        </button>
        <div className="mconn"></div>
        <div className="mchildren railed">
          {/* 4 Vedas */}
          <div className="mnode child">
            <button className="mbtn has-children" onClick={handleToggleNode}>
              <span className="mt">The 4 Vedas</span><span className="ms">Ṛg·Sāma·Yajur·Atharva</span>
              <svg className="mchev" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
            </button>
            <div className="mconn"></div>
            <div className="mchildren railed">
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-rgveda')}><span className="mt">Ṛgveda</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-samaveda')}><span className="mt">Sāmaveda</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-yajurveda')}><span className="mt">Yajurveda</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-atharvaveda')}><span className="mt">Atharvaveda</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
            </div>
          </div>
          {/* 4 Layers */}
          <div className="mnode child">
            <button className="mbtn has-children" onClick={handleToggleNode}>
              <span className="mt">The 4 Layers</span><span className="ms">in every Veda</span>
              <svg className="mchev" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
            </button>
            <div className="mconn"></div>
            <div className="mchildren railed">
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-layers')}><span className="mt">Saṃhitā</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-layers')}><span className="mt">Brāhmaṇa</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-layers')}><span className="mt">Āraṇyaka</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
              <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-upanishads')}><span className="mt">Upaniṣad</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
            </div>
          </div>
          {/* Upanishads */}
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-upanishads')}><span className="mt">Upaniṣads · 108</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
        </div>
      </div>
      ...
      <div className="mnode child" data-level="trunk">
        <button className="mbtn trunk has-children" onClick={handleToggleNode}>
          <span className="mt">SMṚTI</span><span className="ms">Remembered · composed</span>
          <svg className="mchev" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
        </button>
        <div className="mconn"></div>
        <div className="mchildren railed">
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-vedanga')}><span className="mt">Vedāṅga · 6</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-upaveda')}><span className="mt">Upaveda · 4</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-itihasa')}><span className="mt">Itihāsa · 2 epics</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-purana')}><span className="mt">Purāṇa · 18+18</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-dharma')}><span className="mt">Dharmaśāstra</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-agama')}><span className="mt">Āgama · Tantra</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-darsana')}><span className="mt">Darśana · 6</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
          <div className="mnode child"><button className="mbtn leaf jumpable" onClick={() => jumpTo('n-stotra')}><span className="mt">Stotra · Bhakti</span><svg className="mjump" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M7 17 17 7M8 7h9v9"/></svg></button></div>
        </div>
      </div>
...
<div style={{textAlign: "center", marginTop: "18px", fontFamily: "var(--sans)", fontSize: ".72rem", color: "var(--faint)"}}>
  Rule of hierarchy — where Smṛti conflicts with Śruti, Śruti prevails.
</div>
</div>
{/* ==================== TRUNK I : SRUTI ==================== */}{/* ==================== TRUNK I : SRUTI ==================== */}
