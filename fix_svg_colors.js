...
function fixColors(filePath) {
    ...
    content = content.replace(/::selection\{background:var\(--sindoor\);color:#fff\}/, '.about-container ::selection{background:var(--sindoor);color:#fff}');
    content = content.replace(/svg\{display:block;margin:0 auto;min-width:940px\}/, '.mapblock svg{display:block;margin:0 auto;min-width:940px}');
    content = content.replace(/svg text\{font-family:'Josefin Sans',sans-serif\}/, '.mapblock svg text{font-family:\'Josefin Sans\',sans-serif}');
    ...
    // For KnowledgeTree.jsx (CSS scoping)
    if (filePath.includes('KnowledgeTree')) {
        content = content.replace(/\*\{margin:0;padding:0;box-sizing:border-box\}\nhtml\{scroll-behavior:smooth\}\n/, '');
        content = content.replace(/::selection\{background:var\(--sindoor\);color:#fff\}/, '.guide-container ::selection{background:var(--sindoor);color:#fff}');
        content = content.replace(/svg text\{font-family:'Josefin Sans',sans-serif\}/, '.mapblock svg text{font-family:\'Josefin Sans\',sans-serif}');
    }
